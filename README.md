# NS高福岡キャンパス図書管理システム
~2022-10-24Version~<br>
~2022-12-23Version~<br>
2023/02/06Version　メールで貸し出した本の返却を図書コードで返却した場合はメール側のデータが正常に処理できません。　使用しないでください。
***
## このリファレンスの見方
 ...
 * 該当ボタン⇛　なし
 * 実行タイプ　⇛　自動実行
 * 補足情報　⇛　シートを開いてから関数実行までに数秒時間がかかります。関数開始までに処理を行わないでください。
 ***
## DBについて
>　この関数について
* データーベース用シートの更新を行うシートです。
* 現在のところは、1000件/日までの制限がかかっています。
* 実行の際は回数に気をつけてください。
 ...
 * 実行タイプ　⇛ 手動実行
 * 補足情報　⇛　1000件/日までの上限あり　~プログラムの構成上、セルデータの更新のためにA1列から行うので1000冊までが上限になる。~　**処理開始位置を変更することで対応してください**
 * <2022-12-23>処理を変更しました。1000/日の条件は変わりませんが、処理開始位置が新規追加分になります。　**今までの分は内容の変更はありません。**
 ***
## Loadingについて
>　この関数について
* データベースに登録するデータを読み込むシートです。
* ~DB()の関数の1000件/日の制限に加えてこちら側でもほんのタイトルを取得するので、**DB（）と合わせて***1000/日の制限があります。~
* <2022-12-23>処理を変更しました。1000/日の条件は変わりませんが、開始位置が移動して新規追加分のみがデータを引っ張ってきます。
* 登録内容は以下のようになります。
> 図書管理コード（A000から始まる8桁コード）<br>
> ISBN　（本のバーコード　日本図書は基本的に9..又は4..から始まる。*例外あり*）<br>
...
* DB登録に値がない場合はエラーを返却します。（空白の場合は０を入力してください）
 > タイプ
 * 所属シート　⇛DB登録
 * 該当ボタン⇛登録する
 * 実行タイプ　⇛手動実行
...
* こちらの関数は改行などが手動なので、DB汚染だけでなく、正確な情報が入力されない可能性（人的ミス）があります。
* こちらの関数は範囲シートから情報を取得して書き込むだけなので、バーコードをセルに事前に直接入力する必要があります。
 > タイプ
 * 所属シート　⇛大型導入用
 * 該当ボタン⇛登録する
 * 実行タイプ　⇛手動実行
...
***
## write_rog_countについて
* データ登録用シートの登録件数を履歴に書き込みます。
* 内容はDBの取得件数になりますので、消さないでください。
* 最新の履歴が一番上に来ます。
 > タイプ
 * 所属シート　⇛履歴
 * 該当ボタン⇛なし
 * 実行タイプ　⇛big_in(),Register()
 * 補足情報　⇛なし
